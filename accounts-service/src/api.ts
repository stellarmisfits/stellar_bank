...
import {getIssuerByToken, insertInvestor, getUserByEmailAndToken} from './queries';
import stellar_utils from './stellar_utils';
...
api.post('/issuer', async (req, res) => {
  ...
  const [queryError, queryResult] = await to(getIssuerByToken(token));
...
api.post('/register', async (req, res) => {
  ...
  const [queryError, queryResult] = await to(getUserByEmailAndToken(email, token));
  ...
  const [queryIssuerError, queryIssuerResult] = await to(getIssuerByToken(token));
  ...
  const [errorKYC, resultKYC] = await to(axios.post(`http://localhost:3002/api/validate`, {email}));
  ...
  const [authError] = await to(stellar_utils.allowTrustLineAuth(token, publicKey, seed, true));
  ...
  const [insertInvestorError] = await to(insertInvestor(email, token, publicKey, false));
